	...
	u8 *setup[7] = {"1、录入指纹", "2、删除指纹", "3、修改密码", "4、修改时间", "5、录入卡片", "6、删除卡片"};
	...
	int press_FR(void)
{
    ...
    if (ensure == 0x00)
    {
        ...
        if (ensure == 0x00)
        {
            ...
            if (ensure == 0x00 && seach.mathscore >= 40) // 降低阈值至 40 测试
            {
                OLED_Clear_NOupdate();
                Show_Str(20, 10, 128, 24, "解锁中...", 24, 0);
                OLED_Refresh_Gram();
...
void Del_Rfid(void)
{
    u16 ID;
    OLED_Clear();
    Show_Str(0, 0, 128, 16, "=== 删除卡片 ===", 16, 0);
    Show_Str(0, 16, 128, 12, "输入卡片ID (0-9):", 12, 0);
    Show_Str(0, 52, 128, 12, "返回 清空  确认删除", 12, 0);
    OLED_Refresh_Gram();
    delay_ms(50);
    ...
    {
        memset(sys.cardid[ID], 0, 5); // 清空该 ID 的卡片记录
        STMFLASH_Write(SYS_SAVEADDR, (u16 *)&sys, sizeof(sys)); // 保存到 Flash
        OLED_Clear();
        Show_Str(0, 20, 128, 12, "删除卡片成功！", 12, 0);
        OLED_Refresh_Gram();
        delay_ms(1200);
    }
    OLED_Clear();
...
//		delay_ms(3000);
	...
	{
		static u8 last_sec = 0xFF; // 初始化为不可能的值
		u8 tbuf[40];
